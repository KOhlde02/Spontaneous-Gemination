...
def add_root_column(bhs_nouns, strong_nouns):
    ...
    for i, row in bhs_nouns.iterrows():
        ...
        #if not strong_row.empty:  # Check if a matching row exists
        bhs_nouns.loc[i, "Root"] = strong_row.iloc[0]["Root"]  # Safely update the 'Root' value
...
"""
non_gem_nouns = strongs_all_nouns[strongs_all_nouns["Root"].str.get(0) != strongs_all_nouns["Root"].str.get(1)]
...
"""
gutturals = ["א", "ה", "ע", "ר", "ח"]
strong_nouns = non_bicon_nouns[~non_bicon_nouns["Root"].str.get(0).isin(gutturals)]
